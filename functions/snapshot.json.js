 ...
 * внутренней ошибкой соседней панели 838, у которой функция была устроена так же, как
 * здесь: вызов бакета ничем не обёрнут, исключение уходит наружу, и страница вместе со
 * сторожем получают голый 500 при целых данных. Теперь каждый удачный ответ оставляет
 * копию в кэше края, а на отказ бакета функция отдаёт эту копию — с её НАСТОЯЩИМ
 * временем публикации в Last-Modified, так что возраст данных виден и странице, и
 * сторожу, и ничего не выдаётся за свежее. Копии нет — честный 503 с Retry-After вместо
 * сорванного вызова. Пустой бакет копией не маскируется: это настоящее состояние, а не сбой.
 */
...
// Кэш края есть на Cloudflare, включая *.pages.dev (проверено в проде 838 14.09.2026).
// В местном прогоне его нет — тогда функция работает как прежде, без копии.
function edgeCache() {
...
async function serve(context, { withBody }) {
  ...
  // Копия переписывается только при новой версии: снимок меняется раз в двадцать
  // минут, а читают его чаще — класть мегабайт на край на каждый запрос незачем.
