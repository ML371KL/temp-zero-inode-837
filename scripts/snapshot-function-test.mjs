...
//
// 13.09.2026 в 15:39 UTC R2 несколько минут отвечал на чтение внутренней ошибкой соседней
// панели 838, у которой функция была устроена так же, как здесь: вызов бакета ничем не
// обёрнут, исключение уходит наружу, и страница вместе со сторожем получают голый 500 при
// целых данных. Функция исполняется на краю Cloudflare, где местный прогон её не увидит,
// поэтому проверяется здесь — с подменными бакетом и кэшем, но с НАСТОЯЩИМ кодом функции.
//
// Запускается шагом в site.yml ДО публикации: без зелёного теста сайт не выкладывается.
// Руками: node scripts/snapshot-function-test.mjs (нужен Node не старше 22.7 — функция
// лежит в .js без package.json, и её ESM-синтаксис распознаётся автоматически).
...
const PAGE = "https://tzi-837.pages.dev/snapshot.json?t=1";
const V1 = { text: '{"generation":1}', etag: '"v1"', uploaded: new Date("2026-09-14T03:06:24.000Z") };
const V2 = { text: '{"generation":2}', etag: '"v2"', uploaded: new Date("2026-09-14T03:26:24.000Z") };
