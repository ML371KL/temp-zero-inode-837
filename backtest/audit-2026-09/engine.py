...
def composite(df, A, variants=frozenset()):
    ...
    leadS = pd.concat(lead_means, axis=1).mean(axis=1) / 2 * 100 if lead_means else pd.Series(0.0, index=df.index)
    coinS = pd.concat(coin_means, axis=1).mean(axis=1) / 2 * 100 if coin_means else pd.Series(0.0, index=df.index)
